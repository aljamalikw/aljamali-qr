    ...
    <div className={`dashboard-card rounded-2xl border-t-4 p-4 ${accentClass}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="font-serif text-lg font-bold text-white">{customerName}</p>
          <p className="mt-0.5 truncate text-xs text-white/50">
            {TYPE_ICON[order.orderType]} {order.orderType} · Order {order.orderNumber}
          </p>
        </div>
      ...
      {secondaryLabel && (
        <p className="mt-3 truncate text-sm font-medium text-white/85">{secondaryLabel}</p>
      )}
      <p className="mt-0.5 font-serif text-sm font-bold text-gold">
        {formatMoney(order.grandTotal, order.currency || "KWD")}
      </p>
